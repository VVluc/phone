...
export default function Profile() {
  ...
  const onSubmit = async (data: any) => {
    try {
      await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/user/${session?.userId}`, data, {
            ...
            <Card>
              <Card.Body>
                <Grid.Container gap={1.2}>
                  <Grid xs={6}>
                    <Input {...register('fullName')} fullWidth label="Họ tên" color="default" clearable />
                  </Grid>
                  <Grid xs={6}>
                    <Input
                  ...
                  <Grid xs={12}>
                    <Input {...register('address')} fullWidth label="Địa chỉ" color="default" clearable />
                  </Grid>
