import { Badge, Button, Card, Grid, Row, Spacer, Table, Text, User } from '@nextui-org/react';
    ...
    if (order?.paymentMethod === 'COD') {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/order/update-status/${order?.id}`,
        {
          ...
          paidDate: newValue?.value === 'delivered' ? new Date().toISOString() : null,
        },
        {
          headers: {
  ...
  const renderCell = (orderItem: OrderItemType, columnKey: React.Key) => {
    switch (columnKey) {
      case 'product':
        return (
          <User
            squared
            src={
              orderItem.variant.product && orderItem.variant.product.images.length > 0
      ...
      case 'total':
        return (
          <Text b size={13} css={{ tt: 'capitalize', color: '$accents7' }}>
            {(orderItem.orderedQuantity * orderItem.orderedPrice).toLocaleString('vi-VN')}
          </Text>
        );
    }
  };
          ...
          <Card.Header>
            <Row justify="space-between">
              <div>
                ...
                <Text color="$accents7">{order && new Date(order.createdDate).toLocaleString('vi-VN')}</Text>
              </div>
              <div
                style={{
                  display: 'flex',
              ...
              >
                ...
                <Button auto color="secondary" animated={false} icon={<AiFillPrinter size={20} />} />
              </div>
            </Row>
          </Card.Header>
          <Card.Divider />
          <Card.Body>
            <Row>
              <Row css={{ columnGap: 10 }}>
                <FaUserAstronaut size={40} fill="#bd1c0e" />
                <div>
              ...
              <Row css={{ columnGap: 10 }}>
                <BsCreditCardFill size={40} fill="#bd1c0e" />
                <div>
                  <Text b>Phương thức thanh toán</Text>
                  <Text css={{ color: '$accents7' }}>{order?.paymentMethod}</Text>
                </div>
              </Row>
              <Row css={{ columnGap: 10 }}>
                <MdLocationOn size={40} fill="#bd1c0e" />
                <div>
            ...
            <Grid.Container css={{ columnGap: 15 }} wrap="nowrap">
                ...
                <div className="w100">
                  ...
                  >
                    ...
                    <Table.Body items={order?.orderItems || []}>
                      {(item: OrderItemType) => (
                        <Table.Row>{columnKey => <Table.Cell>{renderCell(item, columnKey)}</Table.Cell>}</Table.Row>
                      )}
                    </Table.Body>
                  </Table>
                  ...
                  <Row justify="flex-end">
                    <div style={{ width: 200 }}>
                      <Row justify="space-between">
                        <Text>Phí ship:</Text>
                        <Text>{order?.shippingCost.toLocaleString('vi-VN')} đ</Text>
                      </Row>
                      <Row justify="space-between">
                        <Text>Tổng:</Text>
                      ...
                      <Row justify="space-between">
                        ...
                        <Badge isSquared variant="flat" color={order?.isPaid ? 'success' : 'error'}>
              ...
              <Grid xs={3}>
                {order?.orderStatus === 'delivered' ? (
                  ...
                  <Button onPress={handleShipped} color="secondary" 
                  // shadow
                  >
