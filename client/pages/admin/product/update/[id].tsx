...
export default function Update() {
  ...
  const onSubmit: SubmitHandler<FormValues> = async data => {
    ...
    try {
      ...
      const res = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/admin/product/${product?.id}`, postData, {
        headers: {
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.variants.length === 1 && product.variants[0].attributeValues.length === 0) {
        setIsVariants(false);
              ...
              <Grid.Container css={{ columnGap: 25 }} wrap="nowrap">
                ...
                <Grid xs={8}>
                  <div style={{ width: '100%' }}>
                    ...
                    <Spacer y={1} />
                    <Row>
                      <Input fullWidth label="Slug" {...register('slug')} required />
                    </Row>
                    <Spacer y={1} />
                    <Row>
                      <Textarea fullWidth label="Mô tả" {...register('description')} />
                    </Row>
                    <Spacer y={1} />
                    <Checkbox size="sm" color="secondary" onChange={setIsVariants} isSelected={isVariants}>
                      Có nhiều phiên bản
                    </Checkbox>
                    ...
                    {isVariants ? (
                      <>
                        <Spacer y={1} />
                        <Row align="center">
                          <span style={{ fontSize: '14px' }}>Các phiên bản: &nbsp;</span>
                          <Tooltip
                            content="Thêm"
                            onClick={() =>
                              append({
                    ...
                    <Row>
                      {adding ? (
                        <Button disabled auto bordered color="warning" css={{ px: '$13', minWidth: 129 }}>
...
const ImagePreview = ({ small, control, name, watch, defaultValue }: any) => {
  ...
  return (
    <>
      ...
      >
        {' '}
        <Image src={url} objectFit="cover" width="100%" height="100%" alt="Card image background" />
      </div>
    </>
  );
};
