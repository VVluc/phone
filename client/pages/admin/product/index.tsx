import { Badge, Button, Dropdown, FormElement, Input, Pagination, Row, styled, Table, Text, User } from '@nextui-org/react';
...
const deleteImg = async (imgId: string) => {
  return await axios.delete('/api/images/' + imgId).then((res: any) => res.data);
};
      ...
      <>
        ...
        <AdminLayout title="Sản phẩm">
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Input onChange={handleChange} onKeyUp={handleEnter} aria-label="product-search" placeholder="Tìm kiếm" size="lg" />
            <Button onPress={() => router.push('/admin/product/add')} color="secondary">
              Tạo mới
            </Button>
          </div>

          <div style={{ marginTop: 20 }}>
            <Page keyword={keyword} pageIndex={pageIndex} setPageIndex={setPageIndex} />
            <div style={{ display: 'none' }}>
              <Page keyword={keyword} pageIndex={pageIndex + 1} setPageIndex={setPageIndex} />
            </div>
          </div>
        </AdminLayout>
      </>
...
const Page = memo(
  ({
    ...
    const { data: products, mutate } = useAdminProducts(`?limit=20&page=${pageIndex}&name=${keyword}`, session?.accessToken);
      ...
      Swal.fire({
        ...
        preConfirm: async login => {
          try {
            ...
            const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/admin/product/${id}`, {
              headers: {
    ...
    const renderCell = (product: ProductType, columnKey: React.Key) => {
      switch (columnKey) {
        case 'product':
          return <User squared src={product.images.length > 0 ? product.images[0].url : ''} name={product.name} css={{ p: 0 }} />;

        case 'stock':
          return (
            <Text b size={13} css={{ color: '$accents7' }}>
              {product.variants.reduce((curr, next) => curr + next.quantity, 0)} sản phẩm của {product.variants.length} loại
            </Text>
          );
        ...
        case 'status':
          return (
            <Row>
              {product.isNew && (
                <Badge variant="flat" isSquared color="primary">
                  Mới
                </Badge>
              )}
              {product.isPopular && (
      ...
      <>
        ...
        >
          ...
          <Table.Body items={products?.items || []}>
            {(item: ProductType) => <Table.Row>{columnKey => <Table.Cell>{renderCell(item, columnKey)}</Table.Cell>}</Table.Row>}
          </Table.Body>
        </Table>
        {products && (
