import { Badge, Card, Dropdown, Grid, Row, Table, Text } from '@nextui-org/react';
...
const OrderOverview = () => {
  ...
  >(
    session?.accessToken ? [`${process.env.NEXT_PUBLIC_API_URL}/admin/order/overview`, session?.accessToken] : null,
...
const SalesStatistic = () => {
  ...
  >(
    session?.accessToken
      ? [`${process.env.NEXT_PUBLIC_API_URL}/admin/order/sales-statistic?year=${new Date().getFullYear()}`, session?.accessToken]
      : null,
    (url: string, token: string) =>
      axios
        .get(url, {
...
const LatestOrders = () => {
  ...
  const { data: session } = useSession();
  const { data: orders, mutate } = useAdminOrders(`?limit=5`, session?.accessToken);
  ...
  const renderCell = (order: OrderType, columnKey: React.Key) => {
    switch (columnKey) {
      ...
      case 'items':
        return (
          <Text b size={13} css={{ color: '$accents7' }}>
            {order.orderItems.reduce((curr, next) => curr + next.orderedQuantity, 0)}
          </Text>
        );
      ...
      case 'payment':
        return (
          <Badge isSquared variant="bordered" color={order.isPaid ? 'success' : 'error'}>
      ...
      case 'status':
        return (
          <Badge variant="flat" isSquared color={orderStatusColor(order.orderStatus)}>
            ...
            <Dropdown placement="bottom-right">
              ...
              <Dropdown.Menu aria-label="Static Actions">
                <Dropdown.Item key="edit" textValue="Chi tiết">
                  <Text color="inherit" onClick={() => router.push(`/admin/order/${order.id}`)}>
                    Chi tiết
                  </Text>
                </Dropdown.Item>
  ...
  return (
    ...
    >
      ...
      <Table.Body items={orders?.items || []}>
        {(item: OrderType) => <Table.Row>{columnKey => <Table.Cell>{renderCell(item, columnKey)}</Table.Cell>}</Table.Row>}
      </Table.Body>
    </Table>
  );
};
...
const TopSellingProduct = () => {
  ...
  const { data: topSelling } = useSWR<TopSelling[]>(
    session?.accessToken ? [`${process.env.NEXT_PUBLIC_API_URL}/admin/product/top-selling`, session?.accessToken] : null,
    (url: string, token: string) =>
      axios
        .get(url, {
          headers: { Authorization: 'Bearer ' + token },
      ...
      <tbody>
        {topSelling?.map(to => (
          <tr key={to.name}>
            ...
            <td style={{ borderBottom: '1px solid #f3f3f3', padding: '10px 0' }}>
...
const TotalRevenue = () => {
  ...
  const { data } = useSWR<{
    ...
    session?.accessToken ? [`${process.env.NEXT_PUBLIC_API_URL}/admin/order/total-revenue`, session?.accessToken] : null,
    (url: string, token: string) =>
      axios
        .get(url, {
          headers: { Authorization: 'Bearer ' + token },
...
const TotalOrders = () => {
  ...
  const { data } = useSWR<string>(
    session?.accessToken ? [`${process.env.NEXT_PUBLIC_API_URL}/admin/order/total-order`, session?.accessToken] : null,
    (url: string, token: string) =>
      axios
        .get(url, {
          headers: { Authorization: 'Bearer ' + token },
...
const TotalProduct = () => {
  const { data: session } = useSession();
  const { data } = useSWR<string>(
    session?.accessToken ? [`${process.env.NEXT_PUBLIC_API_URL}/admin/product/total-product`, session?.accessToken] : null,
    (url: string, token: string) =>
      axios
        .get(url, {
          headers: { Authorization: 'Bearer ' + token },
