...
import { validateName, validatePassword, validatePhone } from '../../../../libs/validate';
...
export default function Add() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const defaultOpt = rolesOptions.find(o => o.value === user.roles[0]) || rolesOptions[0];
  ...
  const nameHelper = useMemo(() => {
    ...
    return {
      isValid,
      text: isValid ? 'Correct name' : 'Username must contains at least 6 letter, no space, no special letters',
  ...
  const passHelper = useMemo(() => {
    ...
    return {
      isValid,
      text: isValid ? 'Correct password' : 'Password must contains at least 6 letter, 1 number, uppercase letter',
      color: isValid ? 'success' : 'error',
    };
  }, [password]);
  ...
  const onSubmitAccount: SubmitHandler<AccountFormValues> = async data => {
    ...
    try {
      await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/admin/user/update-account/${id}`, body, {
        headers: {
          Authorization: `Bearer ${session?.accessToken}`,
        },
      });
    ...
    <>
      ...
      <AdminLayout title="Cập nhật nhân viên">
        <Grid.Container gap={2}>
          <Grid xs={12} sm={4}>
            <form style={{ width: '100%' }} onSubmit={inforMethods.handleSubmit(onSubmitInfo)}>
              <Card>
                <Card.Header>
                  <Text b>Thông tin</Text>
                </Card.Header>
                <Card.Body>
                  <Input {...inforMethods.register('fullName')} required label="Họ tên" placeholder="Nhập họ tên" clearable />
                  <Spacer y={1} />
                  <Input {...inforMethods.register('address')} required label="Địa chỉ" placeholder="Nhập địa chỉ" clearable />
                  <Spacer y={1} />
                  <Input
          ...
          <Grid xs={12} sm={4}>
            <FormProvider {...accountMethods}>
              <form style={{ width: '100%' }} onSubmit={accountMethods.handleSubmit(onSubmitAccount)}>
                <Card>
                  <Card.Header>
                  ...
                  <Card.Body>
                    ...
                    <Controller
                      ...
                      rules={{ required: true }}
                      render={({ field }) => (
                        <SelectRole options={rolesOptions} value={field.value} onChange={field.onChange} innerRef={field.ref} />
                      )}
                    />
                    <Spacer y={1} />
                    <Button type="submit" shadow auto color="secondary">
