import { Button, Card, Col, Container, Grid, Row, Table, Text, Tooltip, User } from '@nextui-org/react';
...
export const useCheckVariantIds = (ids: number[]) => {
  const { data } = useSWR<VariantType[]>(ids.length ? [`${process.env.NEXT_PUBLIC_API_URL}/variant/active`, ids] : null, fetcher);
...
export default function Cart() {
  ...
  const renderCell = (variant: CustomCartItemType, columnKey: React.Key) => {
    switch (columnKey) {
      case 'product':
        return (
          <User squared src={variant.product?.images[0].url || ''} name={variant.product?.name} css={{ p: 0 }}>
            <Row>
      ...
      default:
        return (
          <Row justify="center" align="center">
            <Col css={{ d: 'flex' }}>
              <Tooltip content="Xóa" color="error" onClick={() => handleCartRemoveItem(variant.id)}>
                <IconButton>
        ...
        <Container md css={{ mt: 50 }}>
          <Text
            ...
            css={{
              textAlign: 'center',
              color: '#DB2612',
              // textGradient: '45deg, $yellow500 50%, $yellow500 100%',
            }}
            weight="medium"
          >
            GIỎ HÀNG
          ...
          <Grid.Container gap={2}>
            <Grid xs={12} lg={9} css={{ '& .nextui-table-container': { width: '100%' } }}>
              <Table
