import { Badge, Button, Card, Container, Grid, Image, Input, Radio, Row, Spacer, Text, Textarea } from '@nextui-org/react';
      ...
      <Card.Body>
        <div>
          {cartItems.length > 0 ? (
            cartItems.map(cartItem => (
              <Row key={cartItem.id} css={{ columnGap: 15, mb: 25 }} align="center">
                <Badge color="secondary" content={cartItem.amountInCart} shape="rectangle" size="md">
                  <Image width={40} height={40} alt="" src={cartItem.product?.images[0].url || ''} />
                </Badge>
                <div>
                  <Text
                    css={{
                    ...
                    >
                      {cartItem.attributeValues.length > 0 &&
                        cartItem.attributeValues.map(
                          (i, index, arr) => (index ? ', ' : '') + i.value + (index + 1 === arr.length ? ' - ' : '')
                        )}
        ...
        <div>
          {/* <Row css={{ columnGap: 5 }} align="flex-end">
            <Input fullWidth label="Mã giảm giá" placeholder="Nhập mã giảm giá" />
          ...
          <Row css={{ mb: 5 }} justify="space-between">
            ...
            <Text css={{ color: '$accents9' }}>{totalPrice(cartItems)?.toLocaleString('vi-VN')} đ</Text>
          </Row>
          <Row css={{ mb: 5 }} justify="space-between">
          ...
          <Row justify="space-between">
            ...
            <Text size={18} b color="secondary">
              {(totalPrice(cartItems) + shipCost - discount)?.toLocaleString('vi-VN')} đ
            </Text>
          </Row>
      ...
      <Card.Body>
        <Controller
          ...
          render={({ field: { onChange, value, ref } }) => (
            <Radio.Group onChange={onChange} value={value} aria-labelledby="paymentMethod" ref={ref}>
...
export default function Checkout() {
  ...
  const onSubmit: SubmitHandler<FormValues> = async data => {
    ...
    const address = data.address + '. ' + data.city + ', ' + data.district + ', ' + data.ward;
    ...
    try {
      ...
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/order`, postData, {
        headers: {
