import { Button, Card, Checkbox, Col, Input, Row, Spacer, Textarea, useInput } from '@nextui-org/react';
...
export default function CategoryAddForm() {
  ...
  const { value: desValue, reset: resetDes, bindings: desBindings } = useInput('');
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/admin/category`, newItem, {
        headers: {
  ...
  return (
    <Card>
      <Card.Body>
        ...
        />
        <Spacer y={2} />
        <Input value={slug} onChange={e => setSlug(e.target.value)} labelPlaceholder="Slug" clearable />
        <Spacer y={2} />
