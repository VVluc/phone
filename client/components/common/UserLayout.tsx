import { Avatar, Badge, Button, Dropdown, FormElement, Input, Navbar, Row, Spacer, Text } from '@nextui-org/react';
...
const Search = () => {
  ...
  const renderList = () => {
    if (!data)
      return (
        <Row align="center" justify="center" css={{ h: '100%', p: 10, position: 'absolute' }}>
    ...
    <>
      <div className="search-container">
        <Input
          ...
          clearable
          contentLeft={<RiSearchLine fill="var(--nextui-colors-accents6)" size={16} />}
        ...
        <div className={`search-result ${focus ? 'show' : ''}`}>{renderList()}</div>
      </div>
      <style jsx>{`
        .search-container {
          position: relative;
        ...
        >
          {category?.map((item, i) => (
            <Link key={i} href={`/category/${item.slug}`}>
              <Navbar.Link isActive={router.asPath === `/category/${item.slug}`}>{item.name}</Navbar.Link>
            </Link>
          ))}
        </Navbar.Content>
          ...
          {session ? (
              ...
              <Dropdown.Trigger>
                ...
                >
                  {session.avatar ? (
                    ...
                    <HiOutlineUserCircle color="#687076" fontWeight={300} size={30} />
                  )}
                </Button>
              </Dropdown.Trigger>
              <Dropdown.Menu onAction={handleAction} color="secondary" aria-label="Avatar Actions" disabledKeys={['general']}>
                <Dropdown.Item as="div" aria-label="general" key="general" css={{ height: '$18' }}>
                  <Row align="center">
                    {session.avatar ? (
                      <>
                        {' '}
                ...
                <Dropdown.Item key="my_orders" color="default" withDivider>
                  Đơn hàng của tôi
                </Dropdown.Item>
                <Dropdown.Item key="profile" color="default">
                  Thông tin cá nhân
                </Dropdown.Item>
                <Dropdown.Item key="update-password" color="default">
                  Cập nhật mật khẩu
                </Dropdown.Item>
            ...
            isXs && (
              <>
                ...
                <Navbar.Item>
                  <Button onPress={() => router.push('/auth/register')} color="success" auto flat>
                    Đăng ký
                  </Button>
                </Navbar.Item>
              </>
